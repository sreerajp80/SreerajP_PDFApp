...
## Summary of Changes
1. **Appearance Hub**:
   - Removed the 4th navigation card ("App Color") from [AppearanceScreen](lib/features/settings/presentation/appearance_screen.dart).
...
2. **Routes & Screens Cleanup**:
   ...
   - Removed `AppRoute.appColor` and its `/settings/app-color` route from [AppRouter](lib/app/routing/app_router.dart).
   - Deleted `test/features/settings/presentation/app_color_screen_test.dart`.

3. **State & Providers Cleanup**:
   - Removed `lightBaseProvider` and `darkBaseProvider` from [providers.dart](lib/app/config/providers.dart).
   - Removed `prefBaseLight` and `prefBaseDark` from [AppConstants](lib/core/constants/app_constants.dart).
   - Cleaned up base color parameters from `AppTheme` and `ResolvedTheme`.
   - Removed base color references from `PdfApp` in [app.dart](lib/app/app.dart).
